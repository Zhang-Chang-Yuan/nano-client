...
3. **选节点**：在主页节点列表中点选。点「测速」可让内核经各节点实测延迟，
   结果直接显示在列表上；随后可用右上角的排序按钮按延迟或名称排列。
   连接成功后会自动测一轮（可在设置里关掉）。
4. **连接**：点中央电源按钮。内核会在 `127.0.0.1:7890` 开一个 SOCKS5 + HTTP
   混合代理，**并自动接管系统代理**（Linux 用 GNOME 设置、Windows 用 Internet 选项、
   macOS 用 networksetup），断开时自动还原。这样浏览器等程序无需手工配置即可走代理。

   > 桌面端起内核本身只是开了个本地端口，浏览器默认不会走它 —— 所以「自动接管系统代理」
   > 默认开启。若你不需要（例如只想让个别程序走），可以在设置里关掉，再手动把程序指向
   > `127.0.0.1:7890`。

5. **（推荐）改用 TUN 模式做全局接管**：设置 → 代理 → 打开「TUN 模式」，
   点一次「一键授权」输一次系统密码即可，之后免密。

   TUN 在网络层接管流量，**不再需要任何程序支持代理**：

   | | 系统代理 | TUN |
   |---|---|---|
   | 普通程序 | ✅ | ✅ |
   | 不认代理的程序 | ❌ | ✅ |
   | **Flatpak 沙箱应用** | ❌ 读不到系统代理 | ✅ |
   | **`ping`** | ❌ ICMP 不走代理 | ✅ |
   | 需要重启程序 | 是 | 否 |

   授权做的是给内核二进制加 `cap_net_admin,cap_net_raw`（`setcap`），
   不需要整个应用以 root 运行。内核版本更新后会需要重新授权一次。
